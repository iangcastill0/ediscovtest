...
async function reduceIterator(iterator, reducer, initialValue) {
...
async function multipartStreamToBuffer(multipart) {
    return await Promise.all(multipart.map(async (item) => {
        ...
        return {
            ...
            fileStreamBuffer: item.fileStream
                ? await (0, utilsNode_1.readByteStream)(item.fileStream)
                : undefined,
        };
    }));
}
