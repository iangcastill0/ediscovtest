...
test('testUpdatingFolderMetadata', async function testUpdatingFolderMetadata() {
    ...
    const template = await exports.client.metadataTemplates.createMetadataTemplate({
        scope: 'enterprise',
    ...
    const createdMetadata = await exports.client.folderMetadata.createFolderMetadataById(folder.id, 'enterprise', templateKey, {
        ['name']: 'John',
        ['age']: 23,
    ...
    const updatedMetadata = await exports.client.folderMetadata.updateFolderMetadataById(folder.id, 'enterprise', templateKey, [
        {
            op: 'replace',
            path: '/name',
            value: 'Jack',
    ...
    await exports.client.folderMetadata.deleteFolderMetadataById(folder.id, 'enterprise', templateKey);
...
test('testGlobalFolderMetadata', async function testGlobalFolderMetadata() {
    const folder = await (0, commons_generated_js_2.createNewFolder)();
    const folderMetadata = await exports.client.folderMetadata.getFolderMetadata(folder.id);
    if (!(folderMetadata.entries.length == 0)) {
        throw new Error('Assertion failed');
    }
    const createdMetadata = await exports.client.folderMetadata.createFolderMetadataById(folder.id, 'global', 'properties', { ['abc']: 'xyz' });
    ...
    const receivedMetadata = await exports.client.folderMetadata.getFolderMetadataById(folder.id, 'global', 'properties');
    if (!((0, utils_js_2.toString)(receivedMetadata.extraData.abc) == 'xyz')) {
        throw new Error('Assertion failed');
    }
    const newValue = 'bar';
    await exports.client.folderMetadata.updateFolderMetadataById(folder.id, 'global', 'properties', [
        {
            op: 'replace',
            path: '/abc',
            value: newValue,
    ...
    const receivedUpdatedMetadata = await exports.client.folderMetadata.getFolderMetadataById(folder.id, 'global', 'properties');
    if (!((0, utils_js_2.toString)(receivedUpdatedMetadata.extraData.abc) == newValue)) {
        throw new Error('Assertion failed');
    }
    await exports.client.folderMetadata.deleteFolderMetadataById(folder.id, 'global', 'properties');
    await expect(async () => {
        await exports.client.folderMetadata.getFolderMetadataById(folder.id, 'global', 'properties');
    }).rejects.toThrow();
    await exports.client.folders.deleteFolderById(folder.id);
});
