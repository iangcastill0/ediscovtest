...
test('test_get_folder_info', async function test_get_folder_info() {
    const rootFolder = await exports.client.folders.getFolderById('0');
    if (!(rootFolder.id == '0')) {
...
test('test_create_and_delete_folder', async function test_create_and_delete_folder() {
    ...
    const newFolder = await exports.client.folders.createFolder({
        name: newFolderName,
        parent: { id: '0' },
    });
    const createdFolder = await exports.client.folders.getFolderById(newFolder.id);
    if (!(createdFolder.name == newFolderName)) {
        throw new Error('Assertion failed');
    }
    await exports.client.folders.deleteFolderById(newFolder.id);
    await expect(async () => {
        await exports.client.folders.getFolderById(newFolder.id);
    }).rejects.toThrow();
});
test('test_update_folder', async function test_update_folder() {
    const folderToUpdateName = (0, utils_js_1.getUuid)();
    const folderToUpdate = await exports.client.folders.createFolder({
        name: folderToUpdateName,
        parent: { id: '0' },
    });
    const updatedName = (0, utils_js_1.getUuid)();
    const updatedFolder = await exports.client.folders.updateFolderById(folderToUpdate.id, {
        requestBody: {
            name: updatedName,
    ...
    await exports.client.folders.deleteFolderById(updatedFolder.id);
});
test('test_copy_move_folder_and_list_folder_items', async function test_copy_move_folder_and_list_folder_items() {
    ...
    const folderOrigin = await exports.client.folders.createFolder({
        name: folderOriginName,
        parent: { id: '0' },
    });
    const copiedFolderName = (0, utils_js_1.getUuid)();
    const copiedFolder = await exports.client.folders.copyFolder(folderOrigin.id, {
        parent: { id: '0' },
    ...
    const movedFolder = await exports.client.folders.updateFolderById(copiedFolder.id, {
        requestBody: {
            parent: {
                id: folderOrigin.id,
            },
    ...
    const folderItems = await exports.client.folders.getFolderItems(folderOrigin.id);
    ...
    await exports.client.folders.deleteFolderById(movedFolder.id);
    await exports.client.folders.deleteFolderById(folderOrigin.id);
});
