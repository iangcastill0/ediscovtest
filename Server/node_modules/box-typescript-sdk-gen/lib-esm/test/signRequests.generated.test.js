...
test('testCreateGetCancelAndListSignRequest', async function testCreateGetCancelAndListSignRequest() {
    ...
    const fileToSign = await (0, commons_generated_js_1.uploadNewFile)();
    const destinationFolder = await (0, commons_generated_js_2.createNewFolder)();
    const createdSignRequest = await exports.client.signRequests.createSignRequest({
        signers: [
            {
                email: signerEmail,
    ...
    const newSignRequest = await exports.client.signRequests.getSignRequestById(createdSignRequest.id);
    ...
    const cancelledSignRequest = await exports.client.signRequests.cancelSignRequest(createdSignRequest.id);
    if (!((0, utils_js_4.toString)(cancelledSignRequest.status) == 'cancelled')) {
        throw new Error('Assertion failed');
    }
    const signRequests = await exports.client.signRequests.getSignRequests();
    if (!((0, utils_js_4.toString)(signRequests.entries[0].type) == 'sign-request')) {
        throw new Error('Assertion failed');
    }
    await exports.client.folders.deleteFolderById(destinationFolder.id, {
        queryParams: { recursive: true },
    });
    await exports.client.files.deleteFileById(fileToSign.id);
});
