...
import { exportToZipMultiple, exportToPDF, exportToZipMultiplePDF, exportToJSON, exportToZipMultipleJSON, getPublicChannelMessages } from 'utils/apiHelper';
...
const ArchivePrivateChannels = ({ channels, members, teamId, backupId }) => {
    ...
    const goMessages = (event, id, name) => {
        console.log("PrivateChannel ID: ", id, name);
        navigate(`/archive/Slack/${teamId}/private-channel/${name}/${id}/backup/${backupId}`);
    }
    ...
    const handleExport = async () => {
        ...
        for (let i = 0; i < selected.length; i+=1) {
            if (filterBy) {
                ...
                const data = await getPublicChannelMessages(teamId, selected[i], 'archive', startDate.toISOString(), endDate.toISOString(), backupId);
                channels.push(data);
                filenames.push(`${selected[i]}.html`);
            } else {
                const data = await getPublicChannelMessages(teamId, selected[i], 'archive', backupId);
                channels.push(data);
                filenames.push(`${selected[i]}.html`);
            }
        }
    ...
    return (
        <MainCard title="Private Channels" content={false}>
            <CardContent>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell
                                            ...
                                            id={labelId}
                                            scope="row"
                                            onClick={(event) => goMessages(event, row.id, row.name)}
                                            sx={{ cursor: 'pointer' }}
                                        >
                                            <Typography
                                                variant="subtitle1"
                                        ...
                                        <TableCell align="left">{row.num_members}</TableCell>
...
ArchivePrivateChannels.propTypes = {
...
export default ArchivePrivateChannels;
