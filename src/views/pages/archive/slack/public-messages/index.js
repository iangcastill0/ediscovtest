...
import { exportToZip, exportToPDF, exportToJSON, getPublicChannelMessages2, getSlackImage3, getS3PreSignedImageUrl, emojis } from 'utils/apiHelper';
...
const ArchivePublicChannelMessages = () => {
    ...
    const { teamId, name, channelId, backupId } = useParams();
    ...
    React.useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const [data, cursor] = await getPublicChannelMessages2({ teamId, channelId, type: 'archive', backupId, cursor: nextCursor || '' });
                if (isMounted) {
    ...
    // React.useEffect(() => {
    //     if (nextCursor) {
    //         getPublicChannelMessages2({ teamId, channelId, type: 'archive', backupId, cursor: nextCursor }).then(([data, cursor]) => {
    //             setRows(prevRows => [...prevRows, ...data])
    //             setOriginalMessages(prevRows => [...prevRows, ...data])
    //             if (cursor && parseInt(cursor, 10) > parseInt(nextCursor, 10)) {
    //                 setNextCursor(cursor)
                                    ...
                                    >
                                        ...
                                        <TableCell
                                            ...
                                            onClick={(event) =>
                                                goMessages(event, {
                                                    threads: row.threads,
                                                    user: members[row.started_by]
                                                        ? members[row.started_by].display_name ||
                                                        members[row.started_by].real_name
                                                        : row.started_by,
                                                })
                                            }
                                                        ...
                                                        if (e.type === 'emoji') {
                                                            return (
                                                                <span className="slack-emoji" key={idx}>
                                                                    <img
                                                                        className="slack-emoji-img"
                                                                        src={`https://a.slack-edge.com/production-standard-emoji-assets/14.0/google-medium/${e.unicode}.png`}
                                                                        alt={`${e.name}`}
                                                                    />
                                                                </span>
                                                            );
                                                            ...
                                                            {isDownloading[file.id] && filenames.indexOf(file.id) > -1 && (
                                                                <Grid item xs>
                                                                    <LinearProgress
                                                                        variant="determinate"
                                                                        color="secondary"
                                                                        value={downloadProgress[file.id] || 0}
                                                                    />
                                                                </Grid>
                                                            )}
                                                            {isDownloading[file.id] && filenames.indexOf(file.id) > -1 && (
                                        ...
                                        <TableCell
                                            ...
                                            sx={{ cursor: 'pointer', color: 'primary' }}
                                        >
                                            {row.threads && row.threads.length > 0 ? (
                                                <Typography
                                                    variant="subtitle1"
                                        ...
                                        <TableCell align="left">
                                            {row.reactions &&
                                                row.reactions.map((r, idx) => (
                                                    <span style={{ marginRight: '8px' }} key={idx}>
                                                        <OutlinedBadge badgeContent={r.count}>
                                                            <img
                                                                src={`https://a.slack-edge.com/production-standard-emoji-assets/14.0/google-medium/${emojis[r.name]}.png`}
                                                                alt={`${r.name}`}
                                                            />
                                                        </OutlinedBadge>
                                                    </span>
...
export default ArchivePublicChannelMessages;
