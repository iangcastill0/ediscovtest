...
const ConversationsChannel = ({ msgList }) => {
    ...
    const { sharepointData, getConversationsData, getListsData } = React.useContext(MSSharepointContext);
    ...
    const getMails = async (event, row) => {
        setLoading(true);
        if(row.visibility){
            await getConversationsData(accessToken, row.id);
        }
        setLoading(false);
        if(row.visibility){
            setIsFilesView(true);
                                    ...
                                    .map((row, index) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell align="left">{new Date(row.createdDateTime).toLocaleString()}</TableCell>
                                                <TableCell
                                                    component="th"
                                                    id={cellId}
                                                    scope="row"
                                                ...
                                                <TableCell
                                                    ...
                                                    id={cellId}
                                                    scope="row"
                                                    onClick={(event) => getMails(event, row)}
                                                    sx={{ cursor: 'pointer' }}
                                                > 
                    ...
                    <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell> Last Delivered Date </TableCell>
                                {/* <TableCell>  </TableCell> */}
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {sharepointData.conversationList
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                >
                                                    {' '}
                                                    {childRow.uniqueSenders.join(", ")}
                                                </Typography>
                                            </TableCell>
                                            <TableCell>{childRow.topic}</TableCell>
                                            <TableCell>{childRow.preview}</TableCell>
                                            <TableCell>{new Date(childRow.lastDeliveredDateTime).toLocaleString()}</TableCell>
                                        </TableRow>
                                    );
                                })}
                        </TableBody>
...
ConversationsChannel.propTypes = {
