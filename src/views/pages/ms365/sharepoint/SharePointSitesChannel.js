...
const SharePointSitesChannel = ({ msgList }) => {
    ...
    const { sharepointData, getRootFilesData, getListsData } = React.useContext(MSSharepointContext);
    ...
    const getRootFiles = async (event, siteId) => {
        setLoading(true);
        await getRootFilesData(accessToken, siteId);
        setLoading(false);

        setIsFilesView(true);
    }

    const getLists = async (event, siteId) => {
        setLoading(true);
        await getListsData(accessToken, siteId);
        setLoading(false);
    ...
    return (
        <MainCard content={false}>
            ...
            {!isRowView && !isFilesView && !isListsView &&
                <>
                    {/* table */}
                    <TableContainer>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell
                                                    ...
                                                    id={cellId}
                                                    scope="row"
                                                    onClick={(event) => getLists(event, row.id)}
                                                    sx={{ cursor: 'pointer' }}
                                                >
                                                    <ExitToAppOutlinedIcon />
                                                </TableCell>
                                    ...
                                    .map((childRow, childIndex) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell>{childRow.folder ? 'Folder' : 'File'}</TableCell>
                                            </TableRow>
                                        );
                                    })}
                            </TableBody>
            ...
            {isListsView &&
                <>
                    <Button variant="contained" size="large" startIcon={<ArrowBackOutlinedIcon />} onClick={() => setIsListsView(false)}>
                        <Link style={{ color: 'white', textDecoration: 'none' }}>
                            Back
                        </Link>
                    </Button>
                                    ...
                                    .map((childRow, childIndex) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell>{childRow.createdBy.user.displayName}</TableCell>
                                                <TableCell>{childRow.description}</TableCell>
                                                <TableCell>{childRow.list.template}</TableCell>
                                            </TableRow>
                                        );
                                    })}
                            </TableBody>
...
SharePointSitesChannel.propTypes = {
    msgList: PropTypes.array.isRequired,
};

export default SharePointSitesChannel;