...
const SharedWithMeChannel = ({ msgList }) => {
    ...
    const { onedriveData, getFileData } = React.useContext(MSOnedriveContext);
        ...
        if (newString) {
            const newRows = rows.filter((row) => {
                ...
                properties.forEach((property) => {
                    if (row.name.toString().toLowerCase().includes(newString.toString().toLowerCase())) {
                        containsQuery = true;
                    }
                });
    ...
    const getFiles = async (event, row) => {
        console.log("item ID: ", row.id);

        setLoading(true);
        const type = row.remoteItem.folder ? 'Folder' : 'File';

        const driveId = row.remoteItem.parentReference.driveId;
        const remoteId = row.remoteItem.id;
        await getFileData(accessToken, driveId, remoteId, type);
        setLoading(false);
    ...
    return (
        <MainCard content={false}>
                                    ...
                                    .map((row, index) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell>{row.remoteItem.shared.sharedBy.user.displayName}</TableCell>
                                                <TableCell align="left">{new Date(row.remoteItem.shared.sharedDateTime).toLocaleString()}</TableCell>
                                                <TableCell
                                                    component="th"
                                                    id={cellId}
                                                    scope="row"
                                                ...
                                                <TableCell
                                                    ...
                                                    id={cellId}
                                                    scope="row"
                                                    onClick={(event) => getFiles(event, row)}
                                                    sx={{ cursor: 'pointer' }}
                                                >
            ...
            {isFilesView &&
                <>
                    <Button variant="contained" size="large" startIcon={<ArrowBackOutlinedIcon />} onClick={() => setIsFilesView(false)}>
                        <Link style={{ color: 'white', textDecoration: 'none' }}>
                            Back
                        </Link>
                    </Button>
                                    ...
                                    .map((childRow, childIndex) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell>{childRow.createdBy.user.displayName}</TableCell>
                                                <TableCell>{childRow.size}</TableCell>
                                                <TableCell>{new Date(childRow.lastModifiedDateTime).toLocaleString()}</TableCell>
                                                <TableCell>
                                                    <Link href={childRow['@microsoft.graph.downloadUrl']}>
                                                        <FileDownloadIcon />
                                                    </Link>
                                                </TableCell>
                                            </TableRow>
...
SharedWithMeChannel.propTypes = {
    msgList: PropTypes.array.isRequired,
};

export default SharedWithMeChannel;