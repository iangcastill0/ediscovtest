...
import { getMS365UsersApi } from 'utils/apiHelper'
...
const MSUsers = () => {
    ...
    const dispatch = useDispatch();
    const { resetAccessToken } = React.useContext(MS365Context)

    React.useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            const { data } = await getMS365UsersApi(workspaceId)
            setRows(data)
            setLoading(false)
            setOriginalRows(data)
        }
        fetchData()
        resetAccessToken(workspaceId)
    }, [workspaceId, resetAccessToken])
    ...
    const handleSearch = event => {
        ...
        if (newString) {
            const newRows = originalRows.filter(row => {
                ...
                const properties = ['displayName', 'givenName', 'surname', 'mail', 'jobTitle']
    ...
    const goMessages = (event, userId) => {
        // TO-DO
        if (userId) {
            navigate(`/ms365/workspace/${workspaceId}/user/${userId}`)
        } else {
            alert('Invalid User')
        }
    }
    ...
    const onError = () => {
        dispatch(openSnackbar({
            open: true,
            message: 'Microsoft service is unavailable for now. Please try again later',
            variant: 'alert',
            alert: {
                color: 'error'
            },
    ...
    return (
        <MainCard title='Users List' content={false} backButton>
                        ...
                        {stableSort(rows, getComparator(order, orderBy))
                            ...
                            .map((row, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        <TableCell padding='checkbox' sx={{ pl: 3 }} onClick={event => handleClick(event, row._id)}>
                                            <Checkbox
                                                color='primary'
                                        ...
                                        <TableCell
                                            ...
                                            id={labelId}
                                            scope='row'
                                            onClick={event => goMessages(event, row.mail)}
                                            sx={{ cursor: 'pointer' }}
                                        >
                                        ...
                                        {/* <TableCell align="center">
                                            {row.isAuthorized && <Chip label="Authorized" size="small" chipcolor="success" />}
                                            {!row.isAuthorized && <Chip label="Not Authorized" size="small" chipcolor="orange" />}
                                        </TableCell> */}
            ...
            <ArchiveWizard open={openArchiveWizard} workspaceId={workspaceId} users={rows} onClose={() => setOpenArchiveWizard(false)} onArchived={onArchived} onError={onError}/>
            {/* table pagination */}
            <TablePagination
