...
const EventsChannel = ({ msgList }) => {
        ...
        if (newString) {
            const newRows = rows.filter((row) => {
                ...
                properties.forEach((property) => {
                    if (row.summary.toString().toLowerCase().includes(newString.toString().toLowerCase())) {
                        containsQuery = true;
                    }
                });
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelectedId = rows.map((n) => n.id.uniqueQualifier);
    ...
    const handleExportJSON = async () => {
        ...
        setExporting(true);
        for (let i = 0; i < selected.length; i += 1) {
            const selectedObject = rows.find(item => item.id.uniqueQualifier === selected[i]);
                            ...
                            <TableBody>
                                {stableSort(rows, getComparator(order, orderBy))
                                    ...
                                    .map((row, index) => {
                                        ...
                                        const isItemSelected = isSelected(row.id.uniqueQualifier);
                                        ...
                                        return (
                                            ...
                                            >
                                                <TableCell padding="checkbox" sx={{ pl: 3 }} onClick={(event) => handleClick(event, row.id.uniqueQualifier)}>
                                                    <Checkbox
                                                        color="primary"
                                                ...
                                                <TableCell align="left">{new Date(row.start.dateTime).toLocaleString()} ({row.start.timeZone})</TableCell>
                                                <TableCell align="left">{new Date(row.end.dateTime).toLocaleString()} ({row.end.timeZone})</TableCell>
                                                <TableCell
                                                    component="th"
                                                    id={cellId}
                                                    scope="row"
