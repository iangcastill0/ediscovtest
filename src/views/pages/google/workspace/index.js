...
// project imports
import GoogleCard from 'ui-component/cards/GoogleCard'
import Avatar from 'ui-component/extended/Avatar'
import CheckCircleIcon from '@mui/icons-material/CheckCircle'

// assets
import SearchIcon from '@mui/icons-material/Search'
import { getGoogleUsersApi } from 'utils/apiHelper'
...
const GoogleUsers = () => {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            const { data } = await getGoogleUsersApi(workspaceId)
            setRows(data)
            setLoading(false)
    ...
    const handleSearch = event => {
        ...
        if (newString) {
            const newRows = originalRows.filter(row => {
                ...
                const properties = ['displayName', 'mail', 'permission']
    ...
    const goMessages = (event, userId) => {
        // TO-DO
        if (userId) {
            navigate(`/google/workspace/${workspaceId}/user/${userId}?isPersonal=${isPersonal}`)
        } else {
            alert('Invalid User')
        }
    }
    ...
    const onError = () => {
        dispatch(openSnackbar({
            open: true,
            message: 'Google service is unavailable for now. Please try again later',
            variant: 'alert',
            alert: {
                color: 'error'
            },
    ...
    return (
        <GoogleCard title='Users List' content={false} backButton>
            <CardContent>
