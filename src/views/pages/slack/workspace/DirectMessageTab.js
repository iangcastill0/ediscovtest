...
import { getExportAll, sendRequestEmail, exportToJSON, exportToZipMultipleJSON, exportToZipMultiplePDF, exportToZipMultiple } from 'utils/apiHelper';
...
function EnhancedTableHead({ onSelectAllClick, order, orderBy, numSelected, rowCount, onRequestSort, selected, handleExport, handleExportPDF, handleExportJSON, handleRequestAuth, authorizing }) {
            ...
            <TableRow>
                ...
                {numSelected > 0 && (
                    <TableCell padding="none" colSpan={6}>
                        <EnhancedTableToolbar numSelected={selected.length} handleExport={handleExport} handleExportPDF={handleExportPDF} handleExportJSON={handleExportJSON} handleRequestAuth={handleRequestAuth} authorizing={authorizing} />
                    </TableCell>
                )}
                {numSelected <= 0 &&
...
const EnhancedTableToolbar = ({ numSelected, handleExport, handleExportPDF, handleExportJSON, handleRequestAuth, authorizing }) => (
    <Toolbar
        sx={{
            p: 0,
            pl: 1,
...
const DirectMessageTab = ({ teamId, channels }) => {
    ...
    const goMessages = (event, userId, authenticated) => {
        if (authenticated) {
            navigate(`/slack/team/${teamId}/direct-messages/${userId}`);
        }
    }
    ...
    const handleExport = async () => {
        ...
        for (let i = 0; i < selectedUsers.length; i += 1) {
            ...
            if (filterBy) {
                ...
                data = await getExportAll(teamId, selectedUsers[i].id, 'slack', startDate.toISOString(), endDate.toISOString());
            } else {
                data = await getExportAll(teamId, selectedUsers[i].id, 'slack');
            }
            data.forEach((item) => {
    ...
    const handleExportJSON = async () => {
        ...
        for (let i = 0; i < selectedUsers.length; i += 1) {
            if (filterBy) {
                startDate.setHours(0, 0, 0, 0);
                endDate.setHours(23, 59, 59, 0);
                const data = await getExportAll(teamId, selectedUsers[i].id, 'slack', startDate.toISOString(), endDate.toISOString());
                channels.push(data);
                filenames.push(`${selectedUsers[i].display_name || selectedUsers[i].name}.json`);
            } else {
                const data = await getExportAll(teamId, selectedUsers[i].id, 'slack');
                channels.push(data);
                filenames.push(`${selectedUsers[i].display_name || selectedUsers[i].name}.json`);
            }
        }
    ...
    const handleRequestAuth = async () => {
        setAuthorizing(true);
        const selectedUsers = rows.filter((row) => selected.indexOf(row.id) !== -1 && !row.authenticated);
        console.log("Request Authenticated: ", selectedUsers);
        const mailsTo = selectedUsers.map((user) => user.email);
        if (mailsTo.length > 0) {
            await sendRequestEmail(teamId, mailsTo);
        }
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell
                                            ...
                                            id={labelId}
                                            scope="row"
                                            onClick={(event) => goMessages(event, row.id, row.authenticated)}
                                            sx={{ cursor: 'pointer' }}
                                        >
