...
import { exportToZip, exportToPDF, exportToJSON, getPublicChannelMessages2, getSlackImage, emojis } from 'utils/apiHelper';
...
const PublicChannelMessages = () => {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            const data = await getPublicChannelMessages2({ teamId, userId, channelId });
    ...
    React.useEffect(() => {
        const fetchData = async (cursor) => {
            ...
            setLoading(true);
            const data = await getPublicChannelMessages2({ teamId, userId, channelId, cursor });
    ...
    const handleDownload = (teamId, filename, downloadName, fileUrl, size) => {
        startDownload({type: 'Slack', isArchive: false, name: downloadName, id: Date.now(), url: `/slack/download/${teamId}/${filename}?fileUrl=${fileUrl}`, size, responseType: 'blob'})
...
export default PublicChannelMessages;
