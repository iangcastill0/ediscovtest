...
import { exportToZip, exportToPDF, exportToJSON, getConversationDetails2, getSlackImage2, emojis } from 'utils/apiHelper';
...
const ConversationDetails = ({ teamId, userId, channelId, userName }) => {
    ...
    const fetchMessages = async (newCursor) => {
        setLoading(true);
        const data = await getConversationDetails2(teamId, userId, channelId, newCursor);
        ...
        setOriginalMessages(prevRows => [...prevRows, ...data.result]);
    ...
    }, [channelId, teamId, getMembers, members]);
    ...
    const handleExport = async () => {
        ...
        const { result, error } = await exportToZip(selectedMessages, members, 'messages.html', { title: userName, isDirect: true });
        if (!result) {
    ...
    const handleExportPDF = async () => {
        ...
        const { result, error } = await exportToPDF(selectedMessages, members, 'messages.pdf', { title: userName, isDirect: true });
        if (!result) {
            dispatch(openSnackbar({
                open: true,
                message: error,
    ...
    const handleDownload = async (teamId, filename, downloadName, fileUrl, size) => {
        // await saveDownloadState(filename, {pecentCompleted:0, state: "Pending", teamId, fileUrl, downloadName, size});
        startDownload({type: 'Slack', isArchive: false, name: downloadName, id: Date.now(), url: `/slack/download/${teamId}/${filename}?userId=${userId}&fileUrl=${fileUrl}&accessToken=`, size, responseType: 'blob'})
        // setDownloading({ ...isDownloading, [filename]: true });
        // setFileNames([...filenames, filename]);
        // axiosServices({
        //     method: 'GET',
        //     url: `/slack/download/${teamId}/${filename}?fileUrl=${fileUrl}&accessToken=`,
        //     responseType: 'blob',
        ...
        <MainCard title="Conversation Details" content={false} backButton>
                                                        ...
                                                        <MainCard content={false} sx={{ m: '0 auto' }} key={i}>
                                                            <CardMedia
                                                                onClick={async (e) => {
                                                                    e.stopPropagation();
                                                                    setModal(true);
                                                                    const data = await getSlackImage2(row.files, teamId, userId);
                                                                    // const data = await getSlackImage(row.files, teamId, accessToken);
                                                                    setImages(data);
                                                                }
                                                                }
                                                                component="img"
            ...
            <TablePagination
                ...
                labelDisplayedRows={({ from, to, count }) => (
                    <div>
                        {`${from}-${to} of ${count}`}
                        {loading && <CircularProgress size={14} style={{ marginLeft: 10 }} />}
                    </div>
                )}
            />
