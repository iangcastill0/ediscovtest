...
const BoxMembers = () => {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            const resp = await axios.get(`/box/workspace/${workspaceId}/members`)
            if (resp.data?.ok) {
    ...
    const handleSearch = event => {
        ...
        if (newString) {
            const newRows = originalRows.filter(row => {
                ...
                const properties = ['name', 'login', 'created_at', 'modified_at', 'space_amount', 'space_used']
    ...
    const goMessages = (event, userId) => {
        // TO-DO
        if (userId) {
            navigate(`/box/workspace/${workspaceId}/users/${userId}/tabs?isPersonal=${isPersonal}`)
        } else {
            alert('Invalid User')
        }
    }
    ...
    const onError = () => {
        dispatch(openSnackbar({
            open: true,
            message: 'Box service is unavailable for now. Please try again later',
            variant: 'alert',
            alert: {
                color: 'error'
            },
                            ...
                            .map((row, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell>{formatSizeUnits(row.space_amount)}</TableCell>
                                        <TableCell>{formatSizeUnits(row.space_used)}</TableCell>
                                    </TableRow>
                                )
                            })}
                        {emptyRows > 0 && (
