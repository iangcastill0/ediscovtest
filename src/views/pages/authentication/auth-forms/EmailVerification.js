...
import { Box, Button, Grid, Typography, TextField, InputAdornment, IconButton, Snackbar, CircularProgress, MenuItem, FormHelperText, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions, Stack  } from '@mui/material';
...
const EmailVerification = ({ user }) => {
    ...
    const [isStatusSuccess, setIsStatusSuccess] = useState(false); // New state to track success status
    ...
    const handleResend = async () => {
        if (resendTimer === 0) {
            ...
            if (res.data.ok) {
                ...
                setStatusText('Failed to resend. Try again later.');
    ...
    const handleSendCode = async (values, setFieldError, setSubmitting) => {
        setSubmitting(true);
        const res = await sendPhoneNumberVerificationCode(user.email, `+${getCountryCallingCode(values.country || 'US')}${values.phoneNumber}`, values.country || 'US');
        if (res.data.ok) {
        ...
        <Grid container spacing={3} justifyContent="center">
            {showPhoneOtpInput ? (
                <Formik
                    ...
                    onSubmit={(values, { setErrors, setSubmitting }) => handleVerifyPhone(values, setErrors, setSubmitting)}
                >
                    {({ errors, handleBlur, handleChange, handleSubmit, isSubmitting, touched, values, setFieldValue, setFieldError, setErrors, setSubmitting }) => (
                        <Box sx={{ width: '100%', mt: '10px' }}>
                            <Typography variant="h4" align="center" gutterBottom>Phone Number Verification</Typography>
                            <Box mb={2}>
                                <TextField
                                    select
                                    fullWidth
                            ...
                            {phoneCodeSent && (
                                <>
                                    ...
                                    <Box mt={2}>
                                        <Button disableElevation fullWidth size="large" variant="contained" onClick={() => handleVerifyPhone(values, setErrors, setSubmitting)} disabled={isSubmitting || updating}>
                ...
                <Formik
                    ...
                    onSubmit={async (values, { setErrors, setSubmitting }) => {
                        console.log('Email verification form submitted', values);
                        setSubmitting(true);
                ...
                >
                    {({ errors, handleBlur, handleChange, handleSubmit, isSubmitting, touched, values }) => (
                        <form onSubmit={handleSubmit}>
                            <Box sx={{ width: '100%' }}>
                                <Typography variant="h6" align="center" gutterBottom>Email Verification</Typography>
                                <Box mb={2}>
                                    <OtpInput
                                        value={values.otp}
                                ...
                                <Box mb={2}>
                                    <Button disableElevation fullWidth size="large" variant="contained" type="submit" disabled={isSubmitting || updating}>
                                ...
                                <Box mt={2}>
                                    ...
                                    <Typography variant="body1" sx={{ minWidth: 85, ml: 2, textDecoration: 'none', cursor: 'pointer' }} color="primary" onClick={handleResend} disabled={resendTimer > 0}>
