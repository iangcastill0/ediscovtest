...
const Shared = ({ workspaceId, userId, isPersonal }) => {
    ...
    useEffect(() => {
        const fetchDropboxData = async () => {
            setLoading(true);
            try {
                const response = await axiosServices.get(`/dropbox/workspace/${workspaceId}/members/${userId}/files/shared?isPersonal=${isPersonal}`);
